...
public class HardwareCryp {	
	...
	public static void main(String[] args){
		try{
//			System.out.println(sendCmdD10F("10.1.11.102", "6666", "5", "8", "8", "A5A94F22B48A7988A5A94F22B48A7988", "A502016BD4B777CC"));
...
//			String pin= HardwareCryp.getPin(ip, port, timeout, pinkey, data, "0");
...
//			System.out.println(StringUtil.byte2hex("12345678".getBytes()));
	 ...
	 * 根据MacBuff计算Mac
	 * @param ip		硬件加密机的IP(A)
	 * @param port		硬件加密机的端口(A)
	 * @param timeout	超时时间(A)
	 * @param mak		mac密钥密文(H)
	 * @param macbuff	macbuff(H)
	 * @return
	 */
	public static String getMac(String ip,String port,String timeout,String mak,String macbuff) throws Exception {
	 ...
	 * @return
	 */
	public static String pinTransfer(String ip,String port,String timeout,String pik1,String pik2,String acctNo,String oriPinValue,String oriAcctFlag,String destAcctFlag )throws Exception{
		try{
			String oriPinBlock=getPin(ip,port,timeout,pik1,oriPinValue,"0").substring(2);
			
			byte[] oriPinArray=new byte[8];
			if("1".equals(oriAcctFlag)){
				 byte[] panArray=CrypTool.getPanByAcct(acctNo);
				 oriPinArray=CrypTool.getNextMac(StringUtil.hex2Byte(oriPinBlock),panArray);
			 }else{
			...
			String destPinValue=getPin(ip,port,timeout,pik2,StringUtil.byte2hex(destBlockArray),"1").substring(2);
			return destPinValue;
	 ...
	 * @return
	 */
	public static String getMac(String ip,String port,String timeout,String mak,String macbuff,String bankIndex)throws Exception{
	 ...
	 * @return
	 */
	public static String checkMac(String ip,String port,String timeout,String mak,String macbuff)throws Exception{
	 ...
	 * @param port		硬件加密机的端口(A)
	 * @param timeout	超时时间(A)
	 * @param pinkey	PIN密钥,密文(H)
	 * @param data		要加密/解密的数据(H)
	 * @param flag		加解密标志(1,加密；0，解密)
	 * @return
	 */
	public static String getPin(String ip,String port,String timeout,String pinkey,String data,String flag)throws Exception{
		String cypt = "";
		if(pinkey.length()==16){
			cypt = "21";
		...
		String result = flag.equals("0")?sendCmdD114(ip, port, timeout, cypt, pinkey, data):sendCmdD112(ip, port, timeout, cypt, pinkey, data);
		return result.startsWith("41")?"41".concat(result.substring(6)):result;
	}
	 ...
	 * @param port		硬件加密机的端口(A)
	 * @param timeout	超时时间(A)
	 * @param mmklen	次主密钥长度(A)
	 * @param index		次主密钥索引号(A)
	 * @param keytype	密钥类型
	 * (01：通信主密钥;11：PIN加密密钥;12：MAC计算密钥; 13：数据加密密钥;
	 * 	21:CVV计算密钥;22: PVV计算密钥)	 
	 * * @param key		密钥,密文
	 * @return
	 */
	public static String getKey(String ip,String port,String timeout,String mmklen,String index,String keytype,String key)throws Exception{
		String result = sendCmdD102(ip, port, timeout, mmklen, index, keytype, key);		
		if(result.startsWith("41")){
	 ...
	 * @param port		硬件加密机的端口(A)
	 * @param timeout	超时时间(A)
	 * @param keytype	密钥类型(A)
	 * (01：通信主密钥;11：PIN加密密钥;12：MAC计算密钥; 13：数据加密密钥;
	 * 	21:CVV计算密钥;22: PVV计算密钥)
	 * @param key		明文密钥(H)
	 * @return
	 */
	public static String encodeKey(String ip,String port,String timeout,String keytype,String key)throws Exception{
	...
	public static String saveMmk(String ip, String port, String timeout,
		...
		String saveResult=HardwareCryp.sendCmdD10F(ip, port, timeout, "8", index, encodeKey, encedeCheckValue);
	 ...
	 * @param port		加密机端口(A)
	 * @param timeout	超时时间(A)
	 * @param wklen		通信主密钥长度(A)
	 * @param index		通信主密钥索引(A)
	 * @param keytype	工作密钥类型(H)
	 * 	(01：通信主密钥;11：PIN加密密钥;12：MAC计算密钥; 13：数据加密密钥;
	 * 	21:CVV计算密钥;22: PVV计算密钥)
	 * @param wk		通信主密钥加密的工作密钥(H)
	 * @return
	 */
	private static String sendCmdD102(String ip, String port, String timeout,
			String wklen,String index, String keytype, String wk) throws Exception{
...
/*
	private static String sendCmdD104(String ip, String port, String timeout,
		...
		byte[] cmd = StringUtil.hex2Byte("D104");	//命令数据
		byte[] bWk = StringUtil.hex2Byte(wk);		//工作密钥，密文
		byte bWkeyLen = (byte)bWk.length;			//工作密钥长度
		byte bLkLen = Byte.parseByte(wklen);		//通信密钥长度
		byte[] bIndex = StringUtil.short2byte(Integer.parseInt(index));	//通信主密钥索引
...
/*	
	public static String sendCmdD104(String ip, String port, String timeout,
		...
		byte bLkLen = Byte.parseByte(wklen);			//通信密钥长度
		byte[] bIndex = StringUtil.short2byte(Integer.parseInt(index));		//通信主密钥索引
		byte bType = StringUtil.hex2Byte(keytype)[0];	//秘要类型
...
/*
	private static String sendCmdD10C(String ip,String port,String timeout,
		...
		byte[] cmd = StringUtil.hex2Byte("D10C");
		byte keylen = (byte)16;
		byte num = (byte)2;
		byte keytype = StringUtil.hex2Byte("01")[0];
		byte[] dataA = StringUtil.hex2Byte(data1);
		byte[] dataB = StringUtil.hex2Byte(data2);
		byte[] data = new byte[37];
		System.arraycopy(cmd, 0, data, 0, 2);
		data[2] = keylen;
		data[3] = num;
		data[4] = keytype;
		System.arraycopy(dataA, 0, data, 5, 16);
		System.arraycopy(dataB, 0, data, 21, 16);
		return send(ip, port, timeout, data);
	}
*/	
	/**
	 ...
	 * 存储区域主密钥
	 * @param ip		硬件加密机IP(A)
	 * @param port		硬件加密机开放端口(A)
	 * @param timeout	超时时间(A)
	 * @param index		区域主密钥的索引号(H)
	 * @param cklen     校验值长度
	 * @param key		被主密钥加密的区域主密钥(H)
	 * @return
	 */

	private static String sendCmdD10F(String ip,String port,String timeout,
			String checklen,String index,String key,String check)throws Exception{
	 ...
	 * @return
	 */
	private static String sendCmdD132Tmp(String ip,String port,String timeout,
			String mak)throws Exception{
		byte[] cmd = StringUtil.hex2Byte("D132");
		byte bCypt = StringUtil.hex2Byte("02")[0];
		byte[] bMak = StringUtil.hex2Byte(mak);
		byte bMaklen = (byte)bMak.length;
	 ...
	 * @param port		加密机的端口(A)
	 * @param timeout	超时时间(A)
	 * @param mak		MAK密钥，密文(H)
	 * @param mac		待检验的mac长度
	 * @param endata	macbuff数据
	 * @return
	 */
/*
	private static String sendCmdD134(String ip,String port,String timeout,
			String mak,String mac,String endata){
		byte[] cmd = StringUtil.hex2Byte("D134");
		byte bCypt = StringUtil.hex2Byte("02")[0];
		byte[] bMak = StringUtil.hex2Byte(mak);
		byte bMakLen = (byte)bMak.length;		
		byte[] xl = StringUtil.hex2Byte("0000000000000000");
		byte[] bMac = StringUtil.hex2Byte(mac);
		byte[] bEndata = endata.getBytes();
		byte[] enlen = StringUtil.short2byte(bEndata.length);
		byte[] data = new byte[18 + bMakLen + bEndata.length];
		System.arraycopy(cmd, 0, data, 0, 2);
		data[2] = bCypt;
		data[3] = bMakLen;
		System.arraycopy(bMak, 0, data, 4, bMakLen);
		System.arraycopy(xl, 0, data, 4+bMakLen, 8);
		System.arraycopy(bMac, 0, data, 12+bMakLen, 4);
		System.arraycopy(enlen, 0, data, 16 + bMakLen, 2);
		System.arraycopy(bEndata, 0, data, 18 + bMakLen, bEndata.length);
		return send(ip, port, timeout, data);
	}
*/
	/**
	 ...
	 * @return
	 */	
	private static String send(String ip,String port,String timeout,byte[] data)throws Exception{
		Socket socket = null;
		PrintStream ps = null;
		...
		if(!returndata.equals("4501")){
			try {
				...
				if (returndata.startsWith("45")){
					returndata = "4502";
					throw new Exception("Input(HEX):"+StringUtil.byte2hex(data)+"   Output(HEX):"+returndata);
				}
