...
public class CrypTool {
	 ...
	 *            16位十六进制字符串 MAC密钥
	 * @param flag
	 *            ASC 原串采用ASC编码 EBC 原串采用EBC编码 HEX 原串采用HEX编码
	 * @param keyArith
	 *            密钥加密算法：DES-DES算法
	 * @param macArith
	 *            mac加密算法：DES-DES算法
	 ...
	 * 
	 */
	public static String getMac(String macbuff, String mk, String mackey,
			String flag, String keyArith, String macArith) throws Exception {
		...
		try {
			if ("DES".equals(keyArith)) {
				if (mklen == 16) { // 主密钥是双倍DES密钥长度128bit
					mackeybyte = Des.decodeDES2(StringUtil.hex2Byte(mackey
							.trim()), StringUtil.hex2Byte(mk.trim()));
				} else if (mklen == 24) { // 主密钥是3DES密钥长度192bit
					mackeybyte = Des.decodeDES3(StringUtil.hex2Byte(mackey
							.trim()), StringUtil.hex2Byte(mk.trim()));
				} else if (mklen == 8) {
					mackeybyte = Des.decodeDES(StringUtil.hex2Byte(mackey
							.trim()), StringUtil.hex2Byte(mk.trim()));
				}
			}
			...
			if ("DES".equals(macArith)) {
				...
				if (flag.equalsIgnoreCase("EBC")) {
					mac = getMac(Asc2Ebc.ASCIIToEBCDIC(macbuff.getBytes()),
							mackeybyte, macArith);
				}
				if (flag.equalsIgnoreCase("HEX")) {
					mac = getMac(StringUtil.hex2Byte(macbuff), mackeybyte,
							macArith);
				}
	 ...
	 * 
	 */
	public static String getMacReturn8Bytes(String macbuff, String mk,
			String mackey, String flag, String keyArith, String macArith)
			throws Exception {
		int maklen = mackey.trim().length() / 2; // mac密钥的长度（bytes）。
		int mklen = mk.trim().length() / 2; // 主密钥的长度（bytes）
		byte[] mackeybyte = new byte[maklen];
		...
		try {
			...
			if ("DES".equals(macArith)) {
				if (flag.equalsIgnoreCase("ASC")) {
					mac = getMacReturn8Bytes(macbuff.getBytes(), mackeybyte,
							macArith);
				}
				if (flag.equalsIgnoreCase("EBC")) {
					mac = getMacReturn8Bytes(Asc2Ebc.ASCIIToEBCDIC(macbuff
							.getBytes()), mackeybyte, macArith);
				}
				if (flag.equalsIgnoreCase("HEX")) {
					mac = getMacReturn8Bytes(StringUtil.hex2Byte(macbuff),
							mackeybyte, macArith);
				}
			}
	 ...
	 *            次主密钥加密算法，
	 * @param mmk
	 *            次主密钥：32位或者48位十六进制字符串
	 * @param macKeyArith
	 *            mak密钥加密算法，DES-DES算法
	 * @param mackey
	 *            MAC KEY密钥（密文）：16位或者32位十六进制字符串
	 * @param macArith
	 *            mac加密算法，1表示DES，2表示2DES，3表示3DES
	 * @param flag
	 *            ASC 原串采用ASC编码 EBC 原串采用EBC编码
	 * 
	 ...
	 * 
	 */
	public static String genMacReturn8Bytes(String macbuff, String mk,
			String mKeyArith, String mmk, String mmKeyArith, String mackey,
			String macArith, String flag) throws Exception {
		int maklen = mackey.trim().length() / 2; // mac密钥的长度（bytes）。
		int mklen = mk.trim().length() / 2; // 主密钥的长度（bytes）
		...
		try {
			...
			if ("DES".equals(mKeyArith)) {
				if (mklen == 16) { // 主密钥是双倍DES密钥长度128bit
					mmkeybyte = Des.decodeDES2(StringUtil.hex2Byte(mmk.trim()),
							StringUtil.hex2Byte(mk.trim()));
				} else if (mklen == 24) { // 主密钥是3DES密钥长度192bit
					mmkeybyte = Des.decodeDES3(StringUtil.hex2Byte(mmk.trim()),
							StringUtil.hex2Byte(mk.trim()));
				} else if (mklen == 8) {
					mmkeybyte = Des.decodeDES(StringUtil.hex2Byte(mmk.trim()),
							StringUtil.hex2Byte(mk.trim()));
				}
			}
	 ...
	 * 
	 */
	public static String genMac(String macbuff, String mk, String mKeyArith,
			String mmk, String mmKeyArith, String mackey, String macArith,
	 ...
	 */
	private static String getMac(byte[] macbuff, byte[] key, String macArith)
			throws Exception {
		// 填充数据
	 ...
	 * @return 16位十六进制字符串表示的MAC
	 */
	private static String getMacReturn8Bytes(byte[] macbuff, byte[] key,
			String macArith) throws Exception {
		// 填充数据
		int len = macbuff.length;
		if (len % 8 > 0) {
			len = len + (8 - len % 8);
	...
	/**
	 * @author 董保刚 TODO Pin转换，先根据原密钥、算法解密，在根据目标密钥、算法加密
	 * @param oriPinValue
	 *            十六进制表示的原始密文pin
	 * @param accountNo
	 *            帐号
	...
	public static String pinTransfer(String oriPinValue, String accountNo,
		...
		try {
			...
			if ("0".equals(oriEncFlag)) {// 软加密
				...
				oriPinBlock = HardwareCryp.getPin(ip, port, timeout, oriPink,
			...
			if ("1".equals(oriAcctFlag)) {
				...
				oriPinArray = getNextMac(StringUtil.hex2Byte(oriPinBlock),
						panArray);
			} else {
			...
			if ("0".equals(destEncFlag)) {
				...
				transferedPin = HardwareCryp.getPin(ip, port, timeout,
	 ...
	 * @return 明文的KEY
	 */
	public static String getKey(String mk, String mkArith, String mmk,
			String mmkArith, String keybuff) throws Exception {
		int mklen = mk.trim().length() / 2; // 主密钥的长度（bytes）
		int mmklen = mmk.trim().length() / 2; // 次主密钥的长度（bytes）
		...
		if ("DES".equals(mkArith)) {
			if (mklen == 16) { // 主密钥是双倍DES密钥长度128bit
				mmkbyte = Des.decodeDES2(StringUtil.hex2Byte(mmk.trim()),
						StringUtil.hex2Byte(mk.trim()));
			} else if (mklen == 24) { // 主密钥是3DES密钥长度192bit
				mmkbyte = Des.decodeDES3(StringUtil.hex2Byte(mmk.trim()),
						StringUtil.hex2Byte(mk.trim()));
			} else if (mklen == 8) { // 主密钥是DES密钥长度
				mmkbyte = Des.decodeDES(StringUtil.hex2Byte(mmk.trim()),
						StringUtil.hex2Byte(mk.trim()));
			}
		}
		...
		if ("DES".equals(mmkArith)) {
			if (mmklen == 16) { // 双倍DES算法
				byte[] tem = StringUtil.hex2Byte(keybuff);
				key = Des.decodeDES2(tem, mmkbyte);
	 ...
	 * @throws Exception
	 */
	public static String getKeyByMainKey(String mk, String mkArith, String mmk,
			String mmkArith, String keybuff) throws Exception {
		int mklen = mk.trim().length() / 2; // 主密钥的长度（bytes）
		int mmklen = mmk.trim().length() / 2; // 次主密钥的长度（bytes）
		byte[] mmkbyte = new byte[mmklen];
	 ...
	 * @throws Exception
	 */
	public static String encyptDataWithMainKey(String mainKey, String mkArith,
			String key) throws Exception {
		byte[] encodedKey = new byte[0];
		int mklen = mainKey.trim().length() / 2; // 主密钥的长度（bytes）
		if ("DES".equals(mkArith)) {
			if (mklen == 8) {
				encodedKey = Des.encodeDES(StringUtil.hex2Byte(key), StringUtil
						.hex2Byte(mainKey.trim()));
			} else if (mklen == 16) {
				encodedKey = Des.encodeDES2(StringUtil.hex2Byte(key),
						StringUtil.hex2Byte(mainKey.trim()));
			} else if (mklen == 24) {
				encodedKey = Des.encodeDES3(StringUtil.hex2Byte(key),
						StringUtil.hex2Byte(mainKey.trim()));
			}
		}
	 ...
	 * @return
	 */
	public static String genCheckValue(String mk, String mkArith, String value)
			throws Exception {
		...
		if ("DES".equals(mkArith)) {
			if (mklen == 16) { // 主密钥是双倍DES密钥长度128bit
				mmkbyte = Des.decodeDES2(StringUtil.hex2Byte(value.trim()),
						StringUtil.hex2Byte(mk.trim()));

			} else if (mklen == 24) { // 主密钥是3DES密钥长度192bit
				mmkbyte = Des.decodeDES3(StringUtil.hex2Byte(value.trim()),
						StringUtil.hex2Byte(mk.trim()));
			} else if (mklen == 8) { // 主密钥是DES密钥长度
				mmkbyte = Des.decodeDES(StringUtil.hex2Byte(value.trim()),
						StringUtil.hex2Byte(mk.trim()));
	...
	public static void main(String[] args) {
		...
		//			
		// String pinvalue=CrypTool.getMacReturn8Bytes(accountNo, oriMk,
		// oriPink, "ASC", "DES", "DES");
		// // String en=HardwareCryp.getPin("10.0.7.100", "6666", "5",
		// "77492DFBBF28111A4D5EDC5594ED230D", pinvalue, "0");
		...
		// try {
		// String
		// pinvalue=CrypTool.pinTransfer("091FEE56C3A33A32","6210210031700000508","1234567890ABCDEF1234567890ABCDEF","3958546B9447B00F","2","DES","DES","1234567890ABCDEF1234567890ABCDEF","","","DES","DES","10.5.1.140","10000","5","0","0");
		// } catch (Exception e) {
		// // TODO 自动生成 catch 块
		// e.printStackTrace();
		// }
		try {
			...
			String r = CrypTool.genCheckValue("1234567890ABCDEF1234567890ABCDEF", "DES", "DCCDCED4C3887EB8DCCDCED4C3887EB8");
			System.out.println(r);
			
//			String s = CrypTool.getKey("f90c5e3a71bf4d6a08a3ed244d102ac5167be398c076ff69", "DES", "A502016BD4B777CCA502016BD4B777CC", "DES", "D9338FDA");
...
//			// String
//			// pinvalue=CrypTool.genCheckValue("1234567890ABCDEF1234567890ABCDEF","DES","0000000000000000");
//			System.out.println(CrypTool
//					.getMacReturn8Bytes(hexStr, "1234567890ABCDEF",
//							"B8261D45DE497055", "HEX", "DES", "DES"));
//			// System.out.println(StringUtil.byte2hex(Des.encodeDES(StringUtil.hex2Byte("01D39B5B04794026"),
//			// StringUtil.hex2Byte("1234567890ABCDEF"))));
